...
func handleEvent(market *MarketHandler, eventJSON string) (err error) {
	//startTime := time.Now()
	//trace := uuid.NewV4()
	...
	_, err = market.handleEvent(event, eventJSON)
...
func (m MarketHandler) handleNewOrder(event *common.NewOrderEvent) (transaction *models.Transaction, launchLog *models.LaunchLog) {
	...
	if m.orderbook.CanMatch(eventMemoryOrder) {
		matchResult := m.orderbook.ExecuteMatch(eventMemoryOrder, m.market.PriceDecimals)

		if len(matchResult.MatchItems) == 0 {
			log.Errorf("No Match Items, %+v %+v", matchResult, event)
	...
	if eventMemoryOrder.Amount.GreaterThan(decimal.Zero) {
		m.orderbook.InsertOrder(eventMemoryOrder)
		utils.Debug("  [Make Liquidity] price: %s amount: %s (%s)", eventMemoryOrder.Price.StringFixed(5), eventMemoryOrder.Amount.StringFixed(5), eventMemoryOrder.ID)
	}
...
func NewMarket(ctx context.Context, kvStore common.IKVStore, market *models.Market) (*MarketHandler, error) {
	marketOrderbook := NewOrderbook(market.ID)

	marketOrderbook.UsePlugin(func(e *common.OrderbookEvent) {
		marketOrderbook.Sequence = marketOrderbook.Sequence + 1
		_ = sendOrderbookChangeMessage(market.ID, marketOrderbook.Sequence, e.Side, e.Price, e.Amount)
	})
